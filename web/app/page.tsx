    ...
    <main className="relative mx-auto flex w-full max-w-7xl flex-1 flex-col px-6 py-10">
      ...
      >
        <section className="flex flex-col items-start gap-8 text-left">
          ...
          <motion.div variants={fadeInUp} className="grid w-full max-w-2xl grid-cols-1 gap-3 sm:grid-cols-3">
            {METRICS.map((metric) => (
        ...
        <motion.aside variants={fadeInUp} className="editorial-panel relative overflow-hidden rounded-[1.75rem] p-5">
          ...
          <div className="relative">
            ...
            <div className="space-y-3">
              {isError ? (
                <div className="rounded-2xl border border-warn-400/25 bg-warn-glow p-4 text-small text-warn-200">
                  Demo moments are unavailable. Check the FastAPI server.
                </div>
              ) : moments.length === 0 ? (
                <div className="rounded-2xl border border-border-default bg-surface-elevated/50 p-4 text-small text-text-muted">
                  Loading paths...
                </div>
