...
// TODO: Add Zod schemas for portal JSON responses (parity with lib/api.ts) so backend
// shape changes fail fast instead of surfacing as undefined deep in UI.

const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://localhost:8000";
const TOKEN_KEY = "facility_portal_session";
...
type StoredPortalSession = PortalToken & {
...
export function savePortalToken(token: PortalToken) {
  const now = Date.now();
  const stored: StoredPortalSession = {
    ...token,
    access_expires_at: now + token.expires_in_seconds * 1000,
    refresh_expires_at: now + token.refresh_expires_in_seconds * 1000,
  };
...
export async function refreshToken() {
  ...
  const response = await fetch(`${API_BASE_URL}/portal/token/refresh`, {
    method: "POST",
    headers: { "content-type": "application/json" },
...
export async function getCurrentPortalUser() {
  ...
  return parseJson<PortalMe>(response);
}

export async function changePassword(currentPassword: string, newPassword: string) {
  const response = await fetch(`${API_BASE_URL}/portal/password/change`, {
    method: "POST",
...
export async function getUpdateFields() {
  const response = await fetch(`${API_BASE_URL}/portal/updates/fields`, {
    headers: authHeaders(),
  });
  return parseJson<UpdateField[]>(response);
}
