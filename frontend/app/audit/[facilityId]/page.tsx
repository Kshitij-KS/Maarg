...
import { AuditScrollProgress } from "@/components/audit-scroll-progress";
import { AuditTrustScoreAnimated } from "@/components/audit-trust-score";
...
export default async function AuditPage({
  ...
  let evidence;
  try {
    evidence = await getFacilityEvidence(facilityId);
  } catch (err) {
    ...
    <main className="mx-auto w-full max-w-6xl flex-1 space-y-6 px-6 py-8">
      ...
      <div className="glass-panel hairline-top rounded-3xl p-6">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
          ...
          <div className="rounded-2xl border border-border-subtle bg-surface-raised/70 px-5 py-4">
            ...
            <AuditTrustScoreAnimated score={evidence.overall_trust_score} />
          </div>
        </div>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        {evidence.signals.map((signal, index) => {
          ...
          return (
            <MotionReveal key={signal.capability} delay={index * 0.06}>
              <article
                className={cn(
